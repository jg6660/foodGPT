...
export default function Sandbox() {
    ...
    const handleSubmit = async (e) => {
        ...
        const inquiry = previewImage.length>0? `What is the recipe and ingredients for the uploaded dish?`: inputText
        ...
        if(previewImage.length > 0) {
            ...
            newUserItem.content = image_markdown + '\n\n' + `The recipe and ingredients for the uploaded dish are: `
            // newUserItem.content = image_markdown + '\n\n' + newUserItem.content
        ...
        try {
            ...
            let text = ret.result.content || 'An error occured while processing your request. Please Try again'
                // || setCaption('unexpected_error')
                ...
                <div className={`${classes.chat} ${classBorderline}`}>
                    ...
                    {/*<CustomTheme>*/}
                        ...
                        noValidate>
                            ...
                            placeholder={messageItems.length > 1 ? 'Send Reply' : 'Send Message'}
                            // placeholder={setCaption(messageItems.length > 1 ? 'send_reply' : 'send_message')} setCaption(messageItems.length > 1 ? 'send_reply' : 'send_message')
                            disabled={isProcessing}
                            fullWidth
                            inputRef={inputRef}
                            value={inputText}
